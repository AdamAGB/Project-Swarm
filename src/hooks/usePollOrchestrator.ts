...
export function usePollOrchestrator(apiKey: string) {
  ...
  const runPoll = useCallback(async (submission: PollSubmission) => {
    ...
    try {
      ...
      const scoredOptions = await scoreOptions(client, poll);

      // Stage 4: Generate personas (instant, no LLM)
      ...
      const personas = generatePersonas(PERSONA_COUNT, undefined, segmentConfig);
      personasRef.current = personas;

      await new Promise((r) => setTimeout(r, 600));

      // Stage 5: Run decision engine (instant, no LLM) + animate
      setProgress({ stage: 'voting', stageLabel: getStageLabel('voting'), votesCompleted: 0, totalVotes: PERSONA_COUNT });
      const votes = computeVotes(personas, scoredOptions, rng, poll.allowMultiple);
      votesRef.current = votes;

      await animatedVoting.startAnimation();

      // Stage 6: Analyze segments (instant)
      ...
      const segments = analyzeSegments(personas, votes, poll.options);

      await new Promise((r) => setTimeout(r, 400));

      // Stage 7: Generate comments (LLM call #3 or #4)
      ...
      const comments = await generateComments(client, poll, representativePersonas, votes);

      // Stage 8: Generate summary (LLM call #4 or #5)
      ...
      const summary = await generateSummary(client, poll, aggregates, segments);
